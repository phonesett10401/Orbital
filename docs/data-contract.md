...
| `altitude` | number | **metres** above mean sea level | **yes** | `null` means unknown, not zero. **Zero means zero**, and both layers that can be at ground level rely on that: an aircraft reporting `"ground"` and a ship, which is at sea level by definition. For those, `null` would claim the source did not say when it did (D165). |
| `velocity` | number | **metres per second**, `>= 0` | **yes** | Ground speed, not airspeed, for an aircraft. **Orbital speed** for a satellite (~7,660 m/s) - not the speed of the sub-satellite point, because the orbital figure is what every source quotes and what a reader expects (D94). Speed over ground for a ship, converted from the knots AIS transmits; the panel converts it back, because knots is the unit at sea (D165). `null` means unknown - and for a ship that is a real answer rather than a rare one, since AIS encodes "not available" as a number in range and the provider refuses anything faster than a vessel can go. |
...
| `model` | string | ICAO type designator | **yes** | What the source says this object *is*, in its own vocabulary — for an aircraft, `B789`. `null` where the source does not say, which is about a quarter of a live map because OpenSky's `/states/all` carries no type. The frontend reads it to size a marker by wingspan; that reading is the frontend's, and this contract promises only the designator. Always `null` for a satellite: the catalogue's answer is identical for every row we draw, and orbit class is something we derive rather than something the source says (D94). For a ship it is the vessel type in words - `Cargo`, `Tanker`, `Tug` - read from the AIS type code, and `null` for the 13% of the feed with no metadata row (D165). |
| `lastSeen` | string | RFC 3339, UTC, `Z` suffix | no | **When this position was current** - never when we polled. For an aircraft that is when the upstream source observed it. A satellite is never observed: its position is computed, so this is the instant it was propagated for, and is exact rather than an extrapolation. For a ship it is when a receiver heard the AIS message, and it matters more here than anywhere else: this source re-serves positions up to a day old on every poll, so the provider drops them on the way in rather than leaving it to the store's eviction, which cannot win against a source that never forgets (D165). The age of the orbital elements behind it is a different fact and lives in `meta`; putting it here would make the store evict every satellite on the poll that created it (D94). |
| `type` | string | `"aircraft"` \| `"satellite"` \| `"ship"` | no | Which layer the object belongs to. A new value is a scope decision, not a code change (D93, D165), and a test on each side of the wire fails until one is written down. |
